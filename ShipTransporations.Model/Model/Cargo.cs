...
namespace ShipTransportations.Model.Model
{
    public class Cargo
    {
        ...
        public int TypeId { 
            ...
            set {
                if (RepositoryHelper.CargoTypesRepository.ReadAll().All(a => a.TypeId != value))
                    throw new Exception(string.Format("Can't find specified TypeID : {0}.", value));
                _typeId = value;
            }
        }
        public int TripId {
            ...
            set {
                if (RepositoryHelper.TripRepositiry.ReadAll().All(a => a.TripId != value))
                    throw new Exception(string.Format("Can't find specified TripID : {0}.", value));
                _tripId = value;
            }
        }
        ...
        public Cargo(int cargoId, int typeId, int tripId, int number, int weight, 
            double price, double insurancePprice) {
            CargoId = cargoId;
            TypeId = typeId;
            TripId = tripId;
            Number = number;
            Weight = weight;
            Price = price;
            InsurancePrice = insurancePprice;
        }
        public Cargo() { }
